...
/**
 * Sentinel's own UEBA output.
 *
 * Catalogued because a real tenant's report showed BehaviorAnalytics — 0.5
 * GB/day, £61/month — labelled "Likely Defender XDR… eligible for the M365 E5
 * data grant". It is neither. A prefix match on "behavior" caught it alongside
 * the genuine XDR tables BehaviorInfo and BehaviorEntities.
 *
 * These are generated by Sentinel from data already in the workspace, not
 * ingested by any connector, which is why Microsoft's connectors reference
 * omits them entirely and nothing here could resolve them. The E5 grant covers
 * Entra logs, Shadow IT discovery, Purview Information Protection and M365
 * advanced hunting — none of which this is.
 *
 * All three publish Basic: No and Auxiliary/Lake: No, so no tier change can
 * reduce them. The only lever is whether UEBA is worth its output.
 */
const UEBA: CatalogueEntry[] = [
  {
    name: 'BehaviorAnalytics',
    description: 'UEBA enrichment — each event scored against the entity\'s own baseline and its peers.',
    category: 'security', billable: true, lakeCapable: false, basicCapable: false,
    recommendedTier: 'analytics',
    reason: 'The output UEBA exists to produce, and what its detections and investigation graphs read.',
    caveat: 'Not Defender XDR and NOT covered by the E5 data grant, despite the name resembling the XDR behaviour tables. Supports neither Basic nor the Lake tier, so the only way to reduce it is to narrow which entities UEBA scores — or to turn it off.',
  },
  {
    name: 'UserPeerAnalytics',
    description: 'Peer groups UEBA calculates for each user, used to judge whether an action is unusual.',
    category: 'security', billable: true, lakeCapable: false, basicCapable: false,
    recommendedTier: 'analytics',
    reason: 'Reference data BehaviorAnalytics scores against — of little use without it.',
    caveat: 'Supports neither Basic nor the Lake tier. Recomputed periodically, so volume tracks directory size rather than user activity.',
  },
  {
    name: 'UserAccessAnalytics',
    description: 'Access-permission relationships UEBA derives between users and resources.',
    category: 'security', billable: true, lakeCapable: false, basicCapable: false,
    recommendedTier: 'analytics',
    reason: 'Supporting UEBA reference data; low volume in most tenants.',
 ...
 */
/**
 * A guess names a FAMILY and stops.
 *
 * It must never make a billing claim — not "billable", not "free", and above
 * all not "eligible for the E5 data grant". Two of these notes did exactly
 * that, and a real tenant was told £61/month of Sentinel UEBA output was
 * covered by a Microsoft 365 grant it has nothing to do with. A prefix is not
 * evidence of what Microsoft charges for.
 *
 * Billability is not guesswork anyway: the paste carries the customer's own
 * BillableMB figure, so the cost column is measured. There is nothing for a
 * guess to add and a great deal for it to get wrong.
 */
export function guessFromName(name: string): TableGuess | null {
  ...
  // "behavior" was removed from this pattern. BehaviorAnalytics is Sentinel
  // UEBA output, not Defender XDR, and matching it here told a real tenant that
  // £61/month of it was covered by the E5 grant. The genuine XDR behaviour
  // tables — BehaviorInfo and BehaviorEntities — are catalogued by name and
  // never reach a guess.
  if (/^(device|email|identity|alert|cloudapp|urlclick|exposure)/i.test(n)) {
    return { label: 'Defender XDR', category: 'security',
      note: 'Looks like a Defender XDR advanced hunting table. Not one we have verified, so check '
        + 'its plan support and grant eligibility before acting on it.' }
  }
  if (/^(aad|entra)/i.test(n)) {
    return { label: 'Entra ID', category: 'security',
      note: 'Looks like an Entra ID table. Not one we have verified — the E5 grant covers sign-in '
        + 'and audit logs but not every table carrying this prefix.' }
  }
  if (/^app[A-Z]/.test(n)) {
